...
async function ask(history, text) {
  ...
  try {
    await runAgent({ history, root, onEvent: makeHandler() });
  } catch (e) {
...
async function main() {
  ...
  if (promptFlag) {
    await ask(history, promptFlag);
    return;
  }

  console.log(C.bold("\n  Copilot CLI") + C.dim(`  ·  ${MODEL}`));
  console.log(C.dim(`  Рабочая область: ${root}`));
  console.log(C.dim("  Пиши запрос. Команды: /reset — сброс, /exit — выход.\n"));
